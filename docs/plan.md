...
| ID | status | upstream | moonbit target | depends on | parallel with | subagent | acceptance | validation | audit | commit scope |
|---|---|---|---|---|---|---|---|---|---|---|
| P1.0 | todo | package boundary decisions | single `src/terminal` package skeleton and file map | P0.4, P0.6, P0.7 | none | main | one `src/terminal` package exists and the planned file layout matches the mapping table | `moon check && moon test && moon fmt && moon info` | `[R]` main | `feat(parser)` |
