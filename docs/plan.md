...
| P11.A | todo | `src/terminal/c/terminal.zig` core host controls | `StreamTerminal` callback/control surface + tests | P11.0, P9.A, P9.B | none | main + `[W]` | `new/free/vt_write`, runtime effect replacement/clearing, `reset`, `mode_get`, `mode_set`, and host metadata setters land as MoonBit owner methods with upstream non-graphics tests in one green task | `moon check && moon test && moon coverage analyze && moon fmt && moon info` | `[R]` main or reviewer subagent | `feat(c-terminal)` |
| P11.B0 | todo | `src/terminal/c/terminal.zig` viewport/screen substrate | screen-set, scrollback, resize, viewport, and page-pin groundwork + tests | P11.0, P11.A, P9.C | none | main + `[W]` | active/alt screen state, pixel dimensions, total/scrollback row accounting, `resize`, `scroll_viewport`, and page-pin substrate land green and unblock faithful grid refs and typed host queries | `moon check && moon test && moon coverage analyze && moon fmt && moon info` | `[R]` main or reviewer subagent | `feat(c-terminal)` |
