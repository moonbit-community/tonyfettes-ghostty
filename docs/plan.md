...
| P17.B | todo | `font/Atlas.zig`, `font/sprite.zig`, `font/sprite/*` | atlas and sprite-font pure rendering substrate | P17.0 | P17.A | `[W]` | atlas packing/grow/write behavior, sprite canvas, direct sprite face dispatch, and pure draw routines land with translated tests and no platform font dependency; rasterizer-dependent path/curve/line/triangle/arc work remains deferred until an adapter is approved | `moon check && moon test && moon coverage analyze && moon fmt && moon info` | `[R]` main or reviewer subagent | `feat(font)` |
