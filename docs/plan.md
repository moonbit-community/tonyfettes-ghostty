...
| P13.D | done | `PageList.zig`, `Screen.zig`, `page.zig` resize/reflow invariants | PageList resize row IDs, tracked blank rows, and saved cursor owner parity + tests | P11.B, P13.B | none | main | column reflow preserves monotonic scrollback row IDs, tracked blank cursor rows survive trimming, and `?1049` saved primary cursors remap through primary storage while alternate is active | `moon check && moon test && moon coverage analyze && moon fmt && moon info` | `[R]` main | `fix(terminal)` |
...
- Completed P13.B audit lives in:
...
- Completed P13.D audit lives in:
  [2026-05-14-p13-d-pagelist-resize-alignment.md](/Users/haoxiang/Workspace/moonbit/feihaoxiang/ghostty/docs/plans/2026-05-14-p13-d-pagelist-resize-alignment.md)
...
| ID | status | upstream | moonbit target | depends on | parallel with | subagent | acceptance | validation | audit | commit scope |
|---|---|---|---|---|---|---|---|---|---|---|
| P17.0 | todo | `src/font/*`, `src/renderer/*` contracts | font/renderer inventory, package map, dependency policy | P14.C | none | main | scope, package boundaries, harfbuzz workspace-member policy, deferred FFI adapters, and phase split are recorded before implementation starts | doc review | `[R]` main | `docs(font-renderer)` |
| P17.A | todo | `font/Glyph.zig`, `font/Metrics.zig`, `font/CodepointMap.zig`, OpenType table users | pure font values, metrics, codepoint map, and table-adapter surface | P17.0 | P17.B | `[W]` | pure data and metrics behavior lands with translated tests; OpenType table access prefers `moonbit-community/harfbuzz/sfnt` instead of duplicate parsers where the API covers the need | `moon check && moon test && moon coverage analyze && moon fmt && moon info` | `[R]` main or reviewer subagent | `feat(font)` |
| P17.B | todo | `font/Atlas.zig`, `font/sprite.zig`, `font/sprite/*` | atlas and sprite-font pure rendering substrate | P17.0 | P17.A | `[W]` | atlas packing/grow/write behavior and sprite canvas/draw routines land with translated tests and no platform font dependency | `moon check && moon test && moon coverage analyze && moon fmt && moon info` | `[R]` main or reviewer subagent | `feat(font)` |
